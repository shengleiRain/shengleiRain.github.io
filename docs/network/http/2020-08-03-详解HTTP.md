...
HTTP全称为Hypertext Transfer Protocol（超文本传输协议），是一种用于**分布式、协作式和超媒体信息系统**的应用层协议。也是一种**无状态**的协议，通过扩展**请求方法、错误码、请求头**来用于实现不仅限于超文本的许多任务。

> 所谓无状态是指在多次请求中，**协议不要求HTTP服务器保留用户的信息和状态**。也就是说，如果后面的请求需要之前的用户的信息或状态，就必须重新请求重传。但是，web应用实现为了保持HTTP的连接状态，通常会采用**Cookie或者Session技术**。

## HTTP协议的格式

### 请求消息格式

请求消息是由四部分组成：**请求行（request line）、请求头部（request header）、空行和请求数据（主体）（request body）**。

![img](\img\in-post\http\HTTP请求消息结构.png)
...
- (最为常用的) origin-form = absolute-path ["?" query]
...
- **GET**
  ...
  - RFC文档：[RFC 7231 4.3.1.](https://tools.ietf.org/html/rfc7231#section-4.3.1)
  ...
  - 常用的使用的场景：GET方法主要用于根据URL来向服务器端请求指定的资源，而且GET方法**应该只承担从服务器获取资源的任务**，不应该有其他的作用。协议规定这是**安全的方法，并且具有幂等性**。

  - **响应是可缓存的。**

- **HEAD**

  - RFC文档：[RFC 7231 4.3.2](https://tools.ietf.org/html/rfc7231#section-4.3.2)
  ...
  - 常用的使用场景：HEAD方法用于向服务器请求响应头中元信息，类似于GET方法，但是服务器的响应数据中**没有body**.协议规定这是**安全的方法，并且具有幂等性**。

  - **用来测试链接的有效性**

  - **请求头与GET请求的一样，除了payload可能会被省略**

  - **同样是可缓存的**

- **POST**

  - RFC文档：[RFC 7231 4.3.3](https://tools.ietf.org/html/rfc7231#section-4.3.3)
  ...
  - > The POST method requests that the target resource process the representation enclosed in the request according to the resource's own specific semantics.  For example, POST is used for the following functions (among others):
    ...
    >
    > If the result of processing a POST would be equivalent to a representation of an existing resource, an origin server MAY redirect  the user agent to that resource by sending a 303 (See Other) response  with the existing resource's identifier in the Location field.  This has the benefits of providing the user agent a resource identifier  and transferring the representation via a method more amenable t shared caching, though at the cost of an extra request if the user agent does not already have the representation cached.
...
- PUT
  ...
  - RFC文档：[RFC 7231 4.3.4](https://tools.ietf.org/html/rfc7231#section-4.3.4)
  ...
  - 主要应用的场景：用来修改已有的资源。与POST的不同之处在于，**POST是新增处理资源，PUT是修改资源的状态**。

  - **PUT的响应是不需要缓存的**，是**幂等的。**

- **PATCH**

  - RFC文档：[RFC 5789](https://tools.ietf.org/html/rfc5789#section-2)
  ...
  - 主要使用场景：可用来创建新资源或修改现有资源，用来对资源进行**局部修改。**

  - 与PUT的差别在于，PUT是认为上传的版本是服务器上的修改版本，所以替换存储的版本。而PATCH包含了一组指令，描述了如何修改当前服务器上的资源以生成新版本。**主要影响uri的资源，并且有可能影响其他资源。**

  - **PATCH、PUT和POST的使用选择：**

    - patch的文档大小要比新资源数据的大，则使用PUT；
    - 不修改资源，则使用POST
    - POST比较起来更困难，选择使用PATCH或者PUT

  - **既不是安全的也不是幂等的。**

- DELETE

  - RFC文档：[RFC 7231 4.3.5](https://tools.ietf.org/html/rfc7231#section-4.3.5)
...
- **CONNECT**
  ...
  - RFC文档：[RFC 7231 4.3.6](https://tools.ietf.org/html/rfc7231#section-4.3.6)

  - >The CONNECT method requests that the recipient establish a tunnel to the destination origin server identified by the request-target and,if successful, thereafter restrict its behavior to blind forwarding of packets, in both directions, until the tunnel is closed. Tunnels are commonly used to create an end-to-end virtual connection, through one or more proxies, which can then be secured using TLS (Transport Layer Security, [[RFC5246](https://tools.ietf.org/html/rfc5246)]).
    >
    >
    >
    ...
    >
    >
    >A client sending a CONNECT request MUST send the authority form of request-target ([Section 5.3 of [RFC7230\]](https://tools.ietf.org/html/rfc7230#section-5.3)); i.e., the request-target consists of only the host name and port number of the tunnel destination, separated by a colon. For example,
    >
    >
    >
  ...
  - 主要应用场景：建立tunnel隧道。用于代理的场景。

- **OPTIONS**

  - RFC文档：[RFC 7231 4.3.7](https://tools.ietf.org/html/rfc7231#section-4.3.7)
...
- 响应码规范：[RFC 7231](https://tools.ietf.org/html/rfc7231#section-6)
- 1xx：消息已经收到，需要进一步处理，HTTP1.0不支持
  - 100 Continue：上传大文件前使用。由客户端发起请求中携带**Expect:100-continue**触发。
  - 101 switch protocol：协议升级使用。由客户端发起请求中携带**upgrade**触发。
  - 102 processing：有些操作涉及到一系列的操作，服务器已经收到了请求，但是需要进一步处理，暂时没有响应的资源返回
- 2xx：成功处理请求
